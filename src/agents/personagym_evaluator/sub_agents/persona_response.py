...
def create_persona_response_agent(task_name: str) -> Agent:
    ...
    system_prompt = f"""
    You are a sub-agent of the green agent tasked with communicating with the external `PersonaGymAgent` to obtain its responses to {NUM_OF_QUESTIONS} scenario-based questions in order to assess its behaviour.
